...
- **`pos` es la esquina SUPERIOR IZQUIERDA, y los actores miden 96x160**: dos bugs distintos salen de olvidarlo. (a) Un cartel flotante sobre la cabeza va a `actor.pos.y - N`, NO a `actor.pos.y - ALTO_AGENTE - N` — esto último da y negativo y el cartel se dibuja fuera del cuadro (así nació invisible la señal de proximidad del Oráculo, wud#10). (b) Una distancia de proximidad medida entre esquinas (`a.pos.dist(b.pos) < 80`) no significa nada con cajas de 160 de alto: para cuando las esquinas están a 80px los actores ya se están tocando y el `onCollide` ya disparó. Medir de CENTRO a centro y con un radio mayor que las cajas.
- **Una curva hecha de rectángulos se ve como una ESCALERA** salvo que cada segmento cubra el salto hasta el siguiente. Al dibujar el camino (F14) con paso de 16px el escalonado era grosero; el arreglo no es solo bajar el paso, es muestrear la curva en `x` y en `x + paso` y estirar cada rectángulo para tapar ambas alturas (`alto = |Δ| + grosor`), de modo que los tramos se solapen. Con eso, paso 8 da una diagonal continua sin necesidad de polígonos.
- **Parallax sin capas de render propias**: un contenedor `k.add([k.pos(0,0), k.z(...)])` con los hijos posicionados en "espacio de capa" y un único `onUpdate` que hace `capa.pos.x = k.getCamPos().x * (1 - factor)` deja a los hijos en pantalla a `x - camX * factor` — o sea moviéndose a `factor` veces la velocidad del suelo. Un solo update mueve la capa entera. Ojo con el rango: en espacio de capa hay que sembrar desde un valor NEGATIVO (al principio del nivel el borde izquierdo mira espacio negativo) y hasta `anchoNivel * factor + ANCHO`.
- **Sobre los fondos pintados, el color de acento crudo desaparece**: un decorado en `VERDE_OSCURO` al 40% sobre la ciudad del módulo 1 es literalmente invisible. Las capas de decorado tienen que ir con el color VIVO (el mismo que usa el camino), no con el acento del módulo.
